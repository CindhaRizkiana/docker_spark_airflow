...
JDBC_URL := "jdbc:postgresql://${POSTGRES_CONTAINER_NAME}/${POSTGRES_DB}"
JDBC_PROPERTIES := '{"user": "${POSTGRES_USER}", "password": "${POSTGRES_PASSWORD}", "driver": "${SPARK_DRIVER}", "stringtype": "${SPARK_STRINGTYPE}"}'
...
docker-build:
	...
	@echo '__________________________________________________________'
	@docker build -t dataeng/airflow -f ./docker/Dockerfile.airflow .
	@echo '__________________________________________________________'
	@docker build -t dataeng/jupyter -f ./docker/Dockerfile.jupyter .
	@echo '__________________________________________________________'
...
spark-submit-test:
	@docker exec ${SPARK_WORKER_CONTAINER_NAME}-1 \
		...
		/spark-scripts/read-postgres.py
